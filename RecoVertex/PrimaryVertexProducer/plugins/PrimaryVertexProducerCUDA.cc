...
PrimaryVertexProducerCUDA::PrimaryVertexProducerCUDA(const edm::ParameterSet& conf)
  ...
  if (conf.exists("vertexCollections")) {
    ...
    for (std::vector<edm::ParameterSet>::const_iterator algoconf = vertexCollections.begin();
      ...
      } else if (fitterAlgorithm == "WeightedMeanFitterCUDA") {
...
void PrimaryVertexProducerCUDA::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  ////////////////////////////////////////////////////////////////////
  ////Josh comments, for now we're doing this on CPU.
  ...
  */
  ...
  ////////////////////// Fitting on GPU //////////////////////////////
  ////////////////////////////////////////////////////////////////////
  ////Josh comments, this is only on the GPU now, using same fit algo as usual
    ...
    for (std::vector<std::vector<reco::TransientTrack> >::const_iterator iclus = clusters.begin();
      ...
      }else if(weightFit)
      {
        ...
        if (algorithm->useBeamConstraint && validBS && (iclus->size() > 1)) {
            ...
            v = WeightedMeanFitterCUDA::weightedMeanOutlierRejectionBeamSpot(points, *iclus, beamSpot);
            if ((v.positionError().matrix())(2,2) != (WeightedMeanFitterCUDA::startError*WeightedMeanFitterCUDA::startError)) pvs.push_back(v);
           ...
           v = WeightedMeanFitterCUDA::weightedMeanOutlierRejection(points, *iclus);
           if ((v.positionError().matrix())(2,2) != (WeightedMeanFitterCUDA::startError*WeightedMeanFitterCUDA::startError)) pvs.push_back(v); //FIX with constants

        }
      }
      else
        throw VertexException(
            "PrimaryVertexProducer: Something went wrong. You are not using the weighted mean fit andno algorithm was selected.");
